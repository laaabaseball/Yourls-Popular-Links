...
function popularlinks_do_page() {
	...
	function show_top($numdays, $numlinks) {
    	...
    	if ($query) {
    	    ...
    	    // Remove/Comment this if you don't want the percentage bar to be displayed
    		...
    		foreach ( $query as $query_result ) {
    			if ($query_result->clicks > 0) {
        			...
        			// Remove/Comment this if you don't want the percentage bar to be displayed. also remove this from line 61: style="--percentage:' . $percentage . '%"
        			$percentage = ($query_result->clicks / $maxClicks) * 100 + 1; // + 1 is a hack for css to fill the container correctly
    	...
    	echo '<h3><b>Popular Links in the Last '. $numdays . ' Days:</b></h3><br> 
    	            <table border="1" class="popular-links">
                    	<thead>
                        	<tr>
                	            <th>Clicks</th>
                	            <th>Created</th>
...
<style>
    ...
    .popular-links td:nth-child(1) {
        background: -webkit-linear-gradient(left, #1a6c3e var(--percentage), transparent var(--percentage));
        background:    -moz-linear-gradient(left, #1a6c3e var(--percentage), transparent var(--percentage));
        background:     -ms-linear-gradient(left, #1a6c3e var(--percentage), transparent var(--percentage));
        background:      -o-linear-gradient(left, #1a6c3e var(--percentage), transparent var(--percentage));
        background:         linear-gradient(to right, #1a6c3e var(--percentage), transparent var(--percentage));
    }
